...
- **CONNECTING**: 初始注册中，尝试连接服务器
- **CONNECTED**: 已成功注册，正常工作中
- **RECONNECTING**: 检测到连接问题，尝试重新注册
- **DISCONNECTED**: 用户主动退出
...
public class AgentBootstrap {
    private static final int[] RETRY_DELAYS = {1000, 2000, 5000, 10000, 30000};
    
    public Registration registerWithRetry(String registerToken, String hostname, String osType) {
        int retryCount = 0;
        int delayIndex = 0;
        ...
        while (true) {
            try {
                ...
                Registration reg = api.register(registerToken, hostname, osType);
                System.out.println("Agent registered successfully!");
                return reg;
                
            } catch (Exception e) {
                retryCount++;
                int delay = RETRY_DELAYS[Math.min(delayIndex, RETRY_DELAYS.length - 1)];
...
public class AgentMainLoop {
    private volatile boolean needReRegister = false;
    ...
    private void performReRegistration() {
        ...
        Registration newReg = registerWithRetry();
        
        // 更新凭证
        this.agentId = newReg.getAgentId();
        this.agentToken = newReg.getAgentToken();
        taskRunner.updateCredentials(agentId, agentToken);
...
public class AgentService {
    ...
    public RegisterResponse register(RegisterRequest request) {
        // 查询是否已存在相同hostname和osType的agent
        Optional<Agent> existingAgent = agentRepository
            .findByHostnameAndOsType(request.getHostname(), request.getOsType());
        ...
        if (existingAgent.isPresent()) {
            ...
            // 关键：生成新的token，确保服务器重启后agent可以重新建立连接
            agent.setAgentToken(UUID.randomUUID().toString());
            agent = agentRepository.save(agent);
            
            log.info("Agent re-registered: {} - New token generated", agent.getAgentId());
        } else {
            // 全新的agent注册
            agent = createNewAgent(request);
        }
        
        return new RegisterResponse(agent.getAgentId(), agent.getAgentToken());
    }
}
```
...
public class HealthChecker {
    ...
    public boolean checkHealth(Runnable healthCheck) {
        try {
            healthCheck.run();
...
- **内存优化**: 重连状态不保留大量历史数据，仅保留必要的配置信息
- **CPU优化**: 使用指数退避策略，避免频繁重试消耗CPU
- **网络优化**: 连接失败时降低请求频率，避免网络拥塞
